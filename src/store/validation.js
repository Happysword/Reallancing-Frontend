...
export default {
  ...
  minLength(propertyName, len) {
    return data =>
      (!!data && data.length >= len) || `Minimum character length for ${propertyName} is ${len}`;
  },
  ...
  maxLength(propertyName, len) {
    return data =>
      (!!data && data.length <= len) || `Maximum character length for ${propertyName} is ${len}`;
  },
  ...
  required(propertyName) {
    return data => (!!data && data.length > 0) || `${propertyName} is required`;
  },
  ...
  noSpecialCharacters(propertyName, allowEmpty = false) {
    ...
    return data =>
      (!!data && data.search(negativeRegex) === -1) ||
      (!data && allowEmpty) ||
  ...
  validEmail(allowEmpty = false) {
    ...
    return email =>
      (!!email && email.search(validRegex) === 0) ||
      (!email && allowEmpty) ||
  ...
  validMobilePhoneNumber(allowEmpty = false) {
    ...
    // Validate that this Regex matches the full string
    return phone =>
      (!!phone && phone.search(validRegex) === 0) ||
      (!phone && allowEmpty) ||
